#!/usr/bin/env bash
set -Eeuo pipefail
# Standalone Gist: generates the reviewed scripts in a temporary directory.
setup_dir="$(mktemp -d)"
trap 'rm -rf -- "$setup_dir"' EXIT
cat > "$setup_dir/install-gb10-comfyui.sh" <<'GB10_EMBED_install_gb10_comfyui_sh'
#!/usr/bin/env bash
set -Eeuo pipefail
...
GB10_EMBED_install_gb10_comfyui_sh
cat > "$setup_dir/start-comfyui-gb10.sh" <<'GB10_EMBED_start_comfyui_gb10_sh'
#!/usr/bin/env bash
set -Eeuo pipefail
...
GB10_EMBED_start_comfyui_gb10_sh
cat > "$setup_dir/register-vast.sh" <<'GB10_EMBED_register_vast_sh'
#!/usr/bin/env bash
set -Eeuo pipefail
...
GB10_EMBED_register_vast_sh
bash "$setup_dir/install-gb10-comfyui.sh" "$@"
